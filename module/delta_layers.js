...
// ===========================================================================
/*
 * Everything above is delta_layers.js verbatim, minus its IIFE wrapper and its
 * BBPlugin.register block. The loader evaluates this with new Function('ctx', source),
 * which supplies the function scope.
 *
 * Delta Layers already tracked its own teardown in `deletables` and gave its hooks back
 * properly, so load and unload are its original onload and onunload almost unchanged.
 * The one difference: the two settings are created through ctx instead of `new Setting`,
 * and are no longer pushed onto `deletables`, because ctx now owns deleting them. The
 * loader calls unload() first and runs the ctx teardown after it, so the ordering the
 * original relied on still holds.
 */
return {
	id: 'delta_layers',
	title: 'Delta Layers',
	...
	blocked: function () {
		if (typeof isApp !== 'undefined' && !isApp) {
			return 'needs the desktop app, since it reads and writes files next to the texture';
		}
		return null;
	},

	load: function (ctx) {
		ctx.setting(SETTING_ID, {
